...
public class Statement2 extends StatementSecondary {
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Sequence<Tree<StatementLabel>> calls = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(calls);
        calls.add(pos, s2.rep);
        this.rep.assemble(label, calls);
        s.clear();
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        Sequence<Tree<StatementLabel>> calls = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(calls);
        Tree<StatementLabel> r = calls.remove(pos);
        s.rep.transferFrom(r);
        this.rep.assemble(label, calls);
